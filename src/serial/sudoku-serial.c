/**
 * @file    sudoku-serial.c
 * @brief   Solucionador de Sudoku — Implementação Serial
 *
 * Lê um puzzle Sudoku n×n a partir de um ficheiro, resolve-o usando
 * backtracking recursivo e imprime a solução no stdout. O tempo de
 * execução do algoritmo é reportado no stderr usando omp_get_wtime().
 *
 * Compilação:
...
/**
 * @brief Resolve o Sudoku recursivamente por backtracking.
 *
 ...
 * mede o tempo de execução do algoritmo com omp_get_wtime(), invoca o
 * solucionador e imprime o resultado. O tempo é enviado para stderr e a
 * solução (ou a mensagem "Nenhuma solução") para stdout, permitindo que
 * os dois fluxos sejam capturados e analisados separadamente.
 *
